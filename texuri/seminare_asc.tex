...
\begin{multicols}{2}
    ...
    \end{lstlisting}
\end{multicols}
\textbf{Explicație:} în esență, singura diferență este modalitatea de calcul a punctului de mijloc: în prima variantă, este calculată media diferenței dintre capete (start și end) și este însumată punctului de plecare (start), pe când în cea de a doua variantă este făcută cât se poate de direct media celor 2 capete.
\\ \noindent \hl{Problema apare pentru cea de a DOUA variantă dacă ambele valori ale capetelor sunt mari, având posibilitatea de a face overflow; primul 'approach' atenuează această posibilitate complet.}
...
\noindent Intuitiv, vom împărți bilele în grupuri mai mici egale, comparându-le pe acestea recursiv până când se ajunge la cazul trivial (grupa este formată dintr-o singură bilă). \hl{Rezolvarea optimă (cu numărul minim de pași necesari):}
începe cu împărțirea celor 12 bile în grupe de câte 4 - le vom nota A, B și C.
comparăm greutățile A și B:
    dacă A = B,
        atunci bila este în c
    altfel,
        este în grupa cu greutatea mai mică;

\textbf{4.}


\textbf{5.} Se dă un număr natural $x$ pe $N$ biți. Câtă informație am câștigat dacă:
\begin{enumerate}[label=(\alph*)]
    \item ni se spune despre $x$ că are exact două valori "1" în reprezentarea sa binară;
    \item ni se spune despre $x$ că are exact $\frac{N}{2}$ valori "1" în reprezentarea sa binară;
    \item ni se spune despre $x$ că are o secvență continuă de $\frac{N}{4}$ biți de "1" în reprezentarea sa binară (restul biților sunt "0");
    \item ni se spune despre $x$ că are MSB setat la "1";
    \item ni se spune despre $x$ că este impar;
    \item ni se spune despre $x$ că este o putere a lui 2;
    \item ni se spune despre $x$ că are primii $\frac{N}{2}$ biți din reprezentarea sa binară setați la "0";
    \item ni se spune despre $x$ că este un număr prim (aici doar o estimare aproximativă este posibilă);
    \item ni se spune despre $x$ că are în reprezentarea sa binară un număr par de biți setați la "1";
...
\hline
\textbf{Evenimentul/simbolul $\mathbf{x}$} & $\mathbf{A}$ & $\mathbf{B}$ & $\mathbf{C}$ & $\mathbf{D}$ & $\mathbf{E}$ \\
\hline
\textbf{Probabilitatea $P(X = x)$} & $\frac{1}{3}$ & $\frac{1}{4}$ & $\frac{1}{6}$ & $\frac{1}{6}$ & $\frac{1}{12}$ \\
\hline
\end{tabular}
...
\end{tabular}
\]
Elementele $D_{ij}$ sunt date (deci avem 9 biți) iar elementele $P_{ij}$ sunt biți de paritate (deci avem 7 biți redundanți): pe fiecare linie, pe fiecare coloană și pe mesajul total. Biții de paritate sunt "1" dacă avem un număr par de "1" în linie/coloană/mesaj. Cerințe:
\begin{enumerate}[label=(\alph*)]
    \item 
\end{enumerate}
...
\textbf{8.} Sistemul de codare \textit{ISBN-10} pentru cărți folosește 9 cifre plus o cifră de verificare pentru a identifica unic fiecare carte. Rolul cifrei de verificare este dublu: 1) detectează dacă o cifră din cele 10 este scrisă greșit și 2) detectează dacă două cifre consecutive au fost interschimbate. Aceste două erori sunt cele mai comune când codăm astfel de informații pentru cărți. Codarea este următoarea: \[ x_{1}, x_{2}, x_{3}, x_{4}, x_{5}, x_{6}, x_{7}, x_{8}, x_{9}, p,\] unde $x_{1},\dots,x_{9} \in \{0,\dots,9\}$ sunt dați iar $p$ (acesta ar fi $x_{10}$) este calculat astfel încât: \[ \sum_{i=1}^{9}(11 - i)x_{i} + p = 0 mod 11.\]
Cerințe:
\begin{enumerate}[label=(\alph*)]
    ...
    \item ce se întâmplă dacă schimbăm regula pentru cifra de verificare astfel încât $\sum_{i=1}^{9}x_{i}+p = 0 mod 11$? Putem detecta/corecta erori la fel de bine?
\end{enumerate}
...
\textbf{10.} Avem un alfabet cu două simboluri:
% \begin{definition}{}
%
% \end{definition}
...
\textbf{1.} Calculați tabelele de adevăr pentru următoarele expresii logice:
\begin{multicols}{3}
...
\begin{tabular}{|c|c|c||c|}
\multicolumn{4}{|c|}{(a) $X = A + BC$} \\ \hline
A & B & C & X \\ \hline
0 & 0 & 0 & 0 \\ \hline
0 & 0 & 1 & 0 \\ \hline
0 & 1 & 0 & 0 \\ \hline
...
\begin{tabular}{|c|c|c||c|}
\multicolumn{4}{|c|}{(b) $X = A(B + D)$} \\ \hline
A & B & D & X \\ \hline
0 & 0 & 0 & 0 \\ \hline
0 & 0 & 1 & 0 \\ \hline
0 & 1 & 0 & 0 \\ \hline
...
\begin{tabular}{|c|c|c||c|}
\multicolumn{4}{|c|}{(c) $X = (A + B)(A + C)$} \\ \hline
A & B & C & X \\ \hline
0 & 0 & 0 & 0 \\ \hline
0 & 0 & 1 & 0 \\ \hline
0 & 1 & 0 & 0 \\ \hline
...
\begin{tabular}{|c|c|c|c||c|}
\multicolumn{5}{|c|}{(d) $X = \overline{A}(B + C)D$} \\ \hline
A & B & C & D & X \\ \hline
0 & 0 & 0 & 0 & 0 \\ \hline
0 & 0 & 0 & 1 & 0 \\ \hline
0 & 0 & 1 & 0 & 0 \\ \hline
...
\begin{tabular}{|c|c|c|c||c|}
\multicolumn{5}{|c|}{(e) $X = A\overline{B}(C + D)$} \\ \hline
A & B & C & D & X \\ \hline
0 & 0 & 0 & 0 & 0 \\ \hline
0 & 0 & 0 & 1 & 0 \\ \hline
0 & 0 & 1 & 0 & 0 \\ \hline
...
\begin{tabular}{|c|c|c|c||c|}
\multicolumn{5}{|c|}{(f) $X = AB(C \oplus D)$} \\ \hline
A & B & C & D & X \\ \hline
0 & 0 & 0 & 0 & 0 \\ \hline
0 & 0 & 0 & 1 & 0 \\ \hline
0 & 0 & 1 & 0 & 0 \\ \hline
...
\begin{tabular}{|c|c|c|c||c|}
\multicolumn{5}{|c|}{(g) $X = A\overline{B}C\overline{D}$} \\ \hline
A & B & C & D & X \\ \hline
0 & 0 & 0 & 0 & 0 \\ \hline
0 & 0 & 0 & 1 & 0 \\ \hline
0 & 0 & 1 & 0 & 0 \\ \hline
...
\begin{tabular}{|c|c|c|c||c|}
\multicolumn{5}{|c|}{(h) $X = (A\overline{B} + \overline{A}B) + (C\overline{D} + \overline{C}D)$} \\ \hline
A & B & C & D & X \\ \hline
0 & 0 & 0 & 0 & 0 \\ \hline
0 & 0 & 0 & 1 & 1 \\ \hline
0 & 0 & 1 & 0 & 1 \\ \hline
...
\begin{tabular}{|c|c|c|c||c|}
\multicolumn{5}{|c|}{(i) $X = (\overline{A}\hspace{0.1cm}\overline{B} + AB)(\overline{C}\hspace{0.1cm}\overline{D} + CD)$} \\ \hline
A & B & C & D & X \\ \hline
0 & 0 & 0 & 0 & 1 \\ \hline
0 & 0 & 0 & 1 & 0 \\ \hline
0 & 0 & 1 & 0 & 0 \\ \hline
...
\begin{tabular}{|c|c|c|c||c|}
\multicolumn{5}{|c|}{(j) $X = A(A \oplus B)(B \oplus C)D$} \\ \hline
A & B & C & D & X \\ \hline
0 & 0 & 0 & 0 & 0 \\ \hline
0 & 0 & 0 & 1 & 0 \\ \hline
0 & 0 & 1 & 0 & 0 \\ \hline
...
\textbf{2.} Desenați circuitele digitale pentru următoarele expresii logice:
\begin{multicols}{2}
\begin{enumerate}[label=(\alph*)]
    \item $X = (A + B)C$
    \item $X = A + BC + \overline{D}$
    \item $X = AB + \overline{A}\hspace{0.12cm}\overline{C}$
    \item $X = !(A + B)(C + D)\overline{D}$
    \item $X = A \oplus B \oplus CD$
    \item $X = A\overline{B}\oplus CB$
\end{enumerate}
\end{multicols}


\textbf{3.}
Legile de Morgan sunt: $\overline{A+B}=\overline{A}\hspace{0.1cm}\overline{B}$ și $\overline{AB}=\overline{A}+\overline{B}$. Cerințe:
\begin{enumerate}[label=(\alph*)]
    \item verificațicele două legi;
    \item desenați circuitele pentru: $\overline{A+B}$, $\overline{A}\hspace{0.1cm}\overline{B}$, $\overline{AB}$ și $\overline{A}+\overline{B}$;
    \item folosind și cele două legi, calculați/simplificați expresiile:
    \begin{enumerate}[label=i.*]
        \item $\neg(\overline{A}+\overline{B}) = AB$
        \item $\neg(\overline{A}\hspace{0.1cm}\overline{B}) = A + B$
        \item $\overline{A+B+C}$, generalizați la $N$ variabile digitale A
        \item $\overline{ABC}$, generalizați la $N$ variabile digitale A
        \item $\neg(A + B) \cdot \overline{A} \cdot \overline{B}$
        \item $\overline{AB}(\overline{A} + \overline{B})$
        \item ${\overline(A + B)}(\overline{A}+\overline{B})$
        \item $\overline{A} \cdot \overline{B} \cdot \overline{(AB)}$
        \item $C + \overline{CB}$
        \item $\overline{AB}(\overline{A} + B)(\overline{B} + \overline{B})$
        \item $\overline{\overline{(A\overline{B})}\overline{(\overline{B} + C)}}$
        \item $\overline{\overline{(A + \overline{B})}(\overline{B} \cdot C)}$
        \item $\overline{(\overline{A} + C)\overline{(AB)}}$
    \end{enumerate}
\end{enumerate}


\textbf{4.} Simplificați următoarele expresii logice:
\begin{multicols}{2}
\begin{enumerate}[label=(\arabic*)]
    \item $A + 0 \equiv$ \hl{$A$} 
    \item $\overline{A}\cdot 0 \equiv$ \hl{$0$}
    \item $A + \overline{A} \equiv$ \hl{$1$}
    \item $A + A \equiv$ \hl{$A$}
    \item $A + AB \equiv A(1+B) \equiv$ \hl{$A$}
    \item $A + \overline{A}\cdot B \equiv$ \hl{$A + B$}
    \item $A(\overline{A} + B) \equiv$ \hl{$AB$}
    \item $AB + \overline{A}B \equiv B(A + \overline{A})$ \hl{$B$}
    \item $(\overline{A} + \overline{B})(\overline{A} + B) \equiv !A + !A \cdot !B + !A \cdot B + !B \cdot B \equiv !A(! + !B + B) + 0 \equiv$ \hl{$!A$}
    \item $A(A + B + C + ...) \equiv$ \hl{$A$}
    \item fie $f(A, B, C) = A + B + C$
        \begin{itemize}[label=i.*]
            \item $f(A, B, AB) \equiv A(1 + B) + B \equiv$ \hl{$A + B$}
            \item $f(A, B, \overline{A}\cdot\overline{B}) \equiv (A + B) + \overline{(A + B)} \equiv$ \hl{$1$}
            \item $f(A, B, \overline{AB}) \equiv (A + !A) + (B + !B)$ \hl{$1$}
        \end{itemize}
    \item $A + A \cdot \overline{A} \equiv A(1+\overline{A})$ \hl{$A$}
    \item $AB + A\overline{B} \equiv A(B + \overline{B}) \equiv$ \hl{$A$}
    \item $\overline{A} +  B \cdot \overline{A} \equiv !A(1 + B)$ \hl{$!A$}
    \item $(D + \overline{A} + B + \overline{C})B \equiv $ \hl{$B$}
    \item $(A + \overline{B})(A + B) \equiv A + A \cdot \overline{B} + A \cdot B + B \cdot \overline{B} \equiv A(1 + B + \overline{B}) + 0 \equiv $ \hl{$A$}
    \item $C(C + CD) \equiv C^2(1 + D) \equiv$ \hl{$C$}
    \item $A(A + AB) \equiv A^2(1 + B) \equiv$ \hl{$A$}
    \item $\overline{!A + !A} \equiv !(!A) \equiv$ \hl{$A$}
    \item $\overline{A + !A} \equiv !1 \equiv $ \hl{$0$}
    \item $D + (D \cdot \overline{C} \cdot B \cdot A) \equiv $ \hl{$D$} 
    \item $!D \cdot \overline{(DCBA)} \equiv !D(!D + !C + !B + !A) \equiv$ \hl{$D$}
    % aici corect4
    \item $AC + \overline{A} \cdot B + BC \equiv $
    \item $(A + C)(\overline{A} + B)(B + C) \equiv $
    \item $\overline{A} + \overline{B} + A \cdot B \cdot \overline{C} \equiv $
    \item $(A + C)(A \cdot D + A \cdot \overline{D}) + AC + C \equiv $
    \item $\overline{A} \cdot (A + B) + (B + AA)(A + \overline{B}) \equiv $
    \item $(A + B)^2 + (A + B)^3 + A + 3\overline{A} + A^3 \equiv (A + B) + (A + B) + (A + 3\overline{A} + A) \equiv (A + B) + (A + \overline{A}) \equiv A + B + 1 \equiv $ \hl{$1$}
\end{enumerate}
\end{multicols}

\textbf{5.} Multiplexare; cerințe:
\begin{enumerate}[label=(\alph*)]
    \item presupunem că avem un MUX cu două intrări binare și un semnal
\end{enumerate}
